...
$(CMD):
	go get ./modules/$@
	go build -o bin/$@/watch-$@ ./modules/$@

.PHONY: $(TARGET)
$(TARGET): $(GOFILES)
	go build -ldflags "-X main.GitCommit=`git rev-parse --short HEAD` -X main.Version=$(VERSION)" -o open-watch

checkbin: bin/ config/ open-watch
...
pack: checkbin
	...
	@$(foreach var,$(CMD),cp ./bin/$(var)/watch-$(var) ./out/$(var)/bin;)
	...
	tar -C out -zcf open-watch-v$(VERSION).tar.gz .
	@rm -rf out
...
clean:
	...
	@rm -rf ./out
	@rm -rf ./$(TARGET)
	@rm -rf open-watch-v$(VERSION).tar.gz
