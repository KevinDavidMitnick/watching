...
func GetTemplates(c *gin.Context) {
	...
	if limit != -1 && page != -1 {
		dt = db.Falcon.Raw(
			fmt.Sprintf("SELECT * from tpl WHERE tpl_name regexp %s limit %d,%d", q, page, limit)).Order("id  desc").Scan(&templates)
	} else {
		dt = db.Falcon.Where("tpl_name regexp ?", q).Order("id desc").Find(&templates)
