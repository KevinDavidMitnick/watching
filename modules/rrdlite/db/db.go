...
func Execute(queries []string, tx, xTime bool) ([]*Result, error) {
	return rrd.Execute(queries, tx, xTime)
}

func (r Rrd) Execute(queries []string, tx, xTime bool) ([]*Result, error) {
	var allResults []*Result
	var graphItems []*cmodel.GraphItem
	var filename string
	err := func() error {
		for _, q := range queries {
			if q == "" {
...
func Query(queries []string, tx, xTime bool) ([]*cmodel.RRDData, error) {
	return rrd.Query(queries, tx, xTime)
}

func (r Rrd) Query(queries []string, tx, xTime bool) ([]*cmodel.RRDData, error) {
	var result []*cmodel.RRDData
	var err1 error
	var filename string
	...
	for _, q := range queries {
		...
		result, err1 = rrdtool.Fetch(filename, qr.Cf, qr.Start, qr.End, qr.Step)
	}
